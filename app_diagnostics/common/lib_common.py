...
def setup():
    nn = lib_nn.NeuralNetworkLSTM(inputSize=3, hiddenSize=64, numLayers=4,outputSize=3)
    allDicts = lib_config.setup_config()
    nnDict = allDicts['nnData']
    scalarDict = allDicts['scalarData']
    OPCDict = allDicts['OPCData']
    nn.load_state_dict(torch.load(nnDict['nnName']))
    return nn, scalarDict, OPCDict